...
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -   
   subroutine dx_preparedata()
      ...
      if(var==1) then
         call tra_coll2phys(vel_ur,pr, vel_ut,pt, vel_uz,pz)
      
      else if(var==2) then
         call var_coll_curl(vel_ur,vel_ut,vel_uz, c1,c2,c3)
         call tra_coll2phys(c1,pr, c2,pt, c3,pz)

      else if(var==3) then
         call var_coll_curl(vel_ur,vel_ut,vel_uz, c1,c2,c3)
         call tra_coll2phys(c3, pr)
         ...
         vel_uz%Re(:,1:) = 0d0
         vel_uz%Im(:,1:) = 0d0
         call tra_coll2phys(vel_uz, pr)

      else if(var==6) then
         stop ' option 6 not implemented'
!         vel_P2%Re(:,0) = 0d0
!         vel_P2%Im(:,0) = 0d0
!         call tra_coll2phys(vel_P2, pr)
         ...
         call tra_coll2phys(c4,pr, c5,pt, c6,pz)
         ...
         _loop_km_end
         call tra_coll2phys(c4,p21, c5,p22, c6,p23)
         ...
         _loop_km_end
         call tra_coll2phys(c4,p31, c5,p32, c6,p33)
