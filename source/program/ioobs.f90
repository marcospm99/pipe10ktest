...
!**************************************************************************
   ...
   character(256)        :: text,fnameima,dirinp,filinp,filstt
...
!--------------------------------------------------------------------------
   subroutine io_load_state()
      ...
      e=nf90_open(io_statefile, nf90_nowrite, f)   ! Abrimos 
...
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   subroutine io_save_coll(f,id,a)
      ...
      if(mpi_rnk==0) then
         ...
         do r = 1, mpi_sze-1
            pH0 = var_H%pH0_(r)
            pH1 = var_H%pH1_(r)  ! These two things should be really close. 
            mpi_tg = r
...
!--------------------------------------------------------------------------
   subroutine io_save_spectrum()
      ...
      double precision ::  dRe(i_N), dIm(i_N) ! ,d(i_N)
